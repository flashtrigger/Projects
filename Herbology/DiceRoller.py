...
def AtkRoller(theRoll, advantage, elvenAccuracy, critRange):
    ...
    if advantage:
        if elvenAccuracy:
            roll1 = roll(1, 20, theRoll[1][2])
            roll2 = roll(1, 20, theRoll[1][2])
            roll3 = roll(1, 20, theRoll[1][2])
        ...
        thisRoll = roll(1, 20, theRoll[1][2])
    isCrit = (thisRoll - theRoll[1][2] >= critRange)
    theText = theText + str(thisRoll)
    if isCrit:
...
def DamageRoller(theRoll, isCrit):
    ...
    for x in range(1, len(theRoll)):
        damage = roll(theRoll[x][0], theRoll[x][1], theRoll[x][2])
        if isCrit:
            damage = damage + (theRoll[x][0] * theRoll[x][1])
        match = False
    ...
    for each in damageList:
        total = total + each[1]
        text2 = text2 + str(each[1]) + " " + str(each[0]) + " "
